defmodule StreamTestWeb.NestedLive do
  ...
  def mount(_params, _session, socket) do
    {:ok, stream(socket, :thing_collections, [])}
  end

  def render(assigns) do
    ~H"""
    <div id="thing_collections" class="flex flex-row" phx-update="stream">
      <div :for={{dom_id, collection} <- @streams.thing_collections} id={dom_id}>
  ...
  def handle_event("new-thing-collection", _params, socket) do
    collection = ThingCollection.new([])

    {:noreply,
      socket
      |> stream_insert(:thing_collections, collection)
      |> stream(collection_id_to_stream_name(collection.id), collection.things)}
  end
